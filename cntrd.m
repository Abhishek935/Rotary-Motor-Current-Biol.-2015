function out=cntrd(im2,mx,sz,interactive,frame)
...
for i=1:nmx
    ...
    tmp=msk.*im2((mx(i,2)-r+1:mx(i,2)+r),(mx(i,1)-r+1:mx(i,1)+r));
    ...
    if interactive==1
     ...
     colormap 'gray'
     hold on;
     subplot(2,1,1),plot(mx(i,1)+xavg-r,mx(i,2)+yavg-r,'ro')
%      axis image
     hold on
     subplot(2,1,2), plot(frame,norm,'ro-')
